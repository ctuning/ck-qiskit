...
try:
	...
	if __name__ == '__main__':
		...
		parser.add_argument("--number", action="store", default=10, dest="input_number", help="select the number to factor")
		myarg=parser.parse_args()
		N = int(myarg.input_number)
		global _backend, _shots, _tout
		_backend = " "
		_shots = 1
		_tout = 500
		repetitions = 1
		...
		if 'CK_IBM_BACKEND' in os.environ:
		    _backend = os.environ['CK_IBM_BACKEND']
		if 'CK_IBM_REPETITION' in os.environ:
		    _shots = int(os.environ['CK_IBM_REPETITION'])
		if 'CK_IBM_TIMEOUT' in os.environ:
		    _tout = int(os.environ['CK_IBM_TIMEOUT'])
		if 'CK_REPETITIONS' in os.environ:
		    repetitions = int(os.environ['CK_REPETITIONS'])
		...
		if (found is False):
		   _backend = available_backends[0]

#		if verbose: print(Q_program.backend_status(_backend))

#		if verbose: print(Q_program.get_my_credits())

		#--------------------------------------------------------------------------------------------------------------


		#global m metterei una variabile ck per M e n
		for m in range(repetitions):
			...
			print("Factors are = ",factors_found)
